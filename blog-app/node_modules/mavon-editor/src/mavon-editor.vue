...
export default {
    ...
    props: {
        ...
        toolbarsBackground: { // 工具欄背景色
            type: String,
            default: '#ffffff'
        },
        editorBackground: { // TODO: 編輯欄背景色
            type: String,
            default: '#ffffff'
        },
        previewBackground: { // 預覽欄背景色
            type: String,
            default: '#fbfbfb'
        },
        boxShadowStyle: { // 陰影樣式
    ...
    data() {
        return {
            ...
            })(), // props true 展示編輯 false展示預覽
            s_fullScreen: false,// 全屏編輯標誌
            s_help: false,// markdown幫助
            s_html_code: false,// 分欄情況下查看html
            d_help: null,
            d_words: null,
            edit_scroll_height: -1,
            s_readmodel: false,
            s_table_enter: false, // 回車事件是否在表格中執行
            d_history: (() => {
                let temp_array = []
                temp_array.push(this.value)
                return temp_array;
            })(), // 編輯記錄
            d_history_index: 0, // 編輯記錄索引
            currentTimeout: '',
            d_image_file: [],
            d_preview_imgsrc: null, // 圖片預覽地址
            s_external_link: {
    ...
    mounted() {
       ...
       /* windowResize(this); */
        ...
        if (!(typeof $vm.externalLink === 'object' && typeof $vm.externalLink['markdown_css'] === 'function')) {
            // 沒有外部文件要來接管markdown樣式，可以更改markdown樣式。
    ...
    methods: {
        ...
        // 切換htmlcode觸發 （status , val）
        htmlcode(status, val) {
            this.$emit('htmlCode', status, val)
        },
        // 打開 , 關閉 help觸發 （status , val）
        ...
        iRender(toggleChange) {
            ...
            this.$render($vm.d_value, function(res) {
                // render
                $vm.d_render = res;
                // change回調  toggleChange == false 時候觸發change回調
                if (!toggleChange)
                {
        ...
        // 清空上一步 下一步緩存
        $emptyHistory() {
            this.d_history = [this.d_value] // 編輯記錄
            this.d_history_index = 0 // 編輯記錄索引
        }
    },
    watch: {
